...
public class PrintToGoPerStationHandler extends PrinterBase {
	// public static final String DRINKS = "DRINKS";
	// public static final String DRINKS_IN_CHINESE = "饮料";
	// public static final String APPETIZERS = "APPETIZERS";
	// public static final String APPETIZERS_IN_CHINESE = "油锅";
	// public static final String ENTREES = "ENTREES";
	// public static final String ENTREES_IN_CHINESE = "炒锅";

	public PrintToGoPerStationHandler(String location, PrintService printService) {
		super(location, printService);
	}

	public void print(PrintToGoPerStationType printToGoPerStationType, FontSizeType fontSizeType, Client clientGui,
		...
		try {
			...
			int dishWorkstationId = (Integer) (Object) Configuration.getToGoPrinterLocationToPrinter().get(location)
					.get(ConfigKeyDefs.printerLocationToPrinter_dishWorkstationId);
			String dishWorkstationName = Configuration.getToGoPrinterLocationToPrinter().get(location)
					.get(ConfigKeyDefs.printerLocationToPrinter_dishWorkstationName);
			String dishWorkstationChineseName = Configuration.getToGoPrinterLocationToPrinter().get(location)
			...
			switch (printToGoPerStationType) {
			case PRINT_TO_GO_PER_STATION:
				out.write(font12x24_W2H2);
				out.write((dishWorkstationName + "\n\n").getBytes());
				break;

			case PRINT_TO_GO_PER_STATTION_IN_CHINESE:
				out.write(allowChineseFontW2H2);
				out.write((dishWorkstationChineseName + "\n\n").getBytes(CHINESE_ENCODING));
				break;

			case PRINT_TO_GO_PER_STATION_IN_ENGLISH_AND_CHINESE:
			case PRINT_TO_GO_PER_STATION_IN_ENGLISH_AND_CHINESE_2:
			...
			default:
				throw new RuntimeException("Unhandled printToGoPerStationType=" + printToGoPerStationType);
			}
			...
			logger.fine("printToGoPerStationType=" + printToGoPerStationType
			...
			for (OrderItem unvoidedOrderItem : unvoidedOrderItemsWithSubstitutedCouponDish) {
				...
				if (dish.getCategory() == Configuration.getDishCategoryNameToIntegerValue()
				...
				} else if (dish.getToGoStationHasWork(dishWorkstationId)) {
					...
					String printingDishName = printToGoPerStationType == PrintToGoPerStationType.PRINT_TO_GO_PER_STATION
							? dish.getShortName()
						...
						for (Subdish subdish : dishSuiteExpandedSubdishes) {
							...
							if (Client.menu.getActiveDish(subdish.getName()) == null) {
								...
								out.write(("   " + subdish.getQuantityInString(largeQuantity, smallQuantity) + "    "
										+ getSubdishName(printToGoPerStationType, subdish, cart) + '\n')
												.getBytes(CHINESE_ENCODING));
							} else {
					...
					for (Customizer customizer : customizers) {
						...
						case VALID:
							...
							out.write("        (".getBytes());
							out.write(getCustomizerName(printToGoPerStationType, customizer, cart)
									.getBytes(CHINESE_ENCODING));
							out.write(font09x17_W1H1);
						...
						case VOIDED:
							...
							//		+ getCancelledString(printToGoPerStationType) + " *** "
							//		+ getCustomizerName(printToGoPerStationType, customizer, cart);
					...
					for (Exchanger exchanger : exchangers) {
						...
						case VALID:
							...
							out.write(chiCustomizerFont);// W1H1 or W1H2
							out.write("        (".getBytes());
							out.write(getExchangerName(printToGoPerStationType, exchangerDish)
									.getBytes(CHINESE_ENCODING));
							out.write(font09x17_W1H1);
							out.write(customizerFont);
							out.write(")\n".getBytes());
						...
						case VOIDED:
							...
							// String voidedName = largeQuantityPlusPadding + smallQuantityPlusPadding + "*** "
							//		+ getCancelledString(printToGoPerStationType) + " *** "
							//		+ getExchangerName(printToGoPerStationType, exchangerDish);
			...
			for (OrderItem voidedOrderItem : voidedOrderItems) {
				...
				if (dish.getToGoStationHasWork(dishWorkstationId)) {
					...
					String dishName = printToGoPerStationType == PrintToGoPerStationType.PRINT_TO_GO_PER_STATION
							? dish.getShortName()
							: dish.getEnglishAndChineseNames();
					...
					String voidedName = largeQuantityPlusPadding + smallQuantityPlusPadding + "*** "
							+ getCancelledString(printToGoPerStationType) + " *** " + dishName;
	...
	private String getCancelledString(PrintToGoPerStationType printToGoPerStationType) {
		logger.finest("Entered");
		switch (printToGoPerStationType) {
		case PRINT_TO_GO_PER_STATION:
			return CANCELLED;
	...
	private String getCustomizerName(PrintToGoPerStationType printToGoPerStationType, Customizer customizer,
			Cart cart) {
		logger.finest("Entered");
	...
	private String getExchangerName(PrintToGoPerStationType printToGoPerStationType, Dish exchangerDish) {
		logger.finest("Entered");
		switch (printToGoPerStationType) {
		case PRINT_TO_GO_PER_STATION:
	...
	private String getSubdishName(PrintToGoPerStationType printToGoPerStationType, Subdish subdish, Cart cart) {
		logger.finest("Entered");
